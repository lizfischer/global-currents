...
def extract():
    ...
    for row in enumerate(reader):
        if row[1]['ms_no'] != '45':
            ...
            # EC
            # url x y msNumber folioNumber w h primaryColor letter class isError errorType notes
            folio = getFolio(row[1]['URL'])
            if (folio[0].isdigit() or folio[0] =='i'or folio[0] == 'x' or folio[0] =='v'): #try to get rid of non-standard folios
                main_writer.writerow([
                    row[1]['URL'], row[1]['x'], row[1]['y'], getMSNumber(row[1]['URL']), getFolio(row[1]['URL']), row[1]['w'],
                    row[1]['h'], row[1]['primary_color'].strip().lower(), row[1]['letter'].strip().upper(), row[1]['class'], getError(row), row[1]['error_type'], 
